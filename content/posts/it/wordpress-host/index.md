...
- домен для инстанса `wp.lxd`;
- домен сайта будет `test1.wp.local`;
- имя администратора для Wordpress `admin`;

## Может пригодиться

- TODO: [Как создать обычного пользователя, если есть только root]( {{< relref "creating-non-root-user" >}} )  
- TODO: [Часто используемые команды MySQL]( {{< relref "dumps" >}} )  
- TODO: [Генерируем стойкий пароль]( {{< relref "generate-strong-password" >}} )
...
TODO в отдельную статью generating strong password и ссылку на нее в раздел модет пригодиться
> 🖉 **NOTE**  
